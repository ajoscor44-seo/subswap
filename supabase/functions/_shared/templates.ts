...
<table width="100%" style="max-width:560px;" cellpadding="0" cellspacing="0">
  ...
  <tr><td style="background:#fafafe;padding:20px 40px;border:1.5px solid #f0eef9;border-top:none;border-radius:0 0 20px 20px;text-align:center;">
    <p style="margin:0 0 5px;font-size:11px;color:#c4b5fd;">${footer}</p>
...
function btn(label: string, url: string) {
  return `<table cellpadding="0" cellspacing="0" style="margin:24px 0;"><tr>
    <td style="background:linear-gradient(135deg,#7c5cfc,#6366f1);border-radius:12px;">
      <a href="${url}" style="display:block;padding:15px 32px;font-size:13px;font-weight:700;color:#fff;text-decoration:none;letter-spacing:0.05em;text-transform:uppercase;">${label} →</a>
    </td></tr></table>`;
}
...
function infoRow(label: string, value: string) {
  return `<tr>
    <td style="padding:10px 14px;font-size:12px;color:#9b8fc2;border-bottom:1px solid #fafafe;">${label}</td>
    <td style="padding:10px 14px;font-size:12px;font-weight:700;color:#1a1230;text-align:right;border-bottom:1px solid #fafafe;">${value}</td>
  </tr>`;
}
...
export function verificationEmail(url: string) {
  return layout(
    ...
    <p style="margin:0;font-size:11px;color:#b8addb;word-break:break-all;background:#fafafe;padding:10px 14px;border-radius:8px;border:1px solid #f0eef9;">${url}</p>
    ${divider()}
...
export function welcomeEmail(username: string) {
  return layout(`
    ...
    <p style="margin:0 0 20px;font-size:14px;color:#9b8fc2;line-height:1.7;">
      Hey <strong style="color:#1a1230;">@${username}</strong> — your account is verified. You can now browse and purchase premium subscription slots at a fraction of the retail price.
    </p>
...
export function purchaseEmail(opts: {
  ...
  return layout(`
    ...
    <p style="margin:0 0 20px;font-size:14px;color:#9b8fc2;line-height:1.7;">
      Hey <strong style="color:#1a1230;">@${opts.username}</strong> — purchase successful. Here are your credentials.
    </p>
    ...
    <table cellpadding="0" cellspacing="0" style="width:100%;background:linear-gradient(135deg,#1a1230,#2d1f6e);border-radius:16px;margin-bottom:24px;">
      <tr><td style="padding:24px 28px;">
        ...
        <p style="margin:0 0 20px;font-size:22px;font-weight:800;color:#fff;">${opts.serviceName}</p>
        <table cellpadding="0" cellspacing="0" style="width:100%;">
          <tr><td style="padding:10px 14px;background:rgba(255,255,255,0.06);border-radius:8px 8px 0 0;">
            ...
            <p style="margin:0;font-size:14px;font-weight:700;color:#fff;">${opts.masterEmail}</p>
          </td></tr>
          <tr><td style="padding:10px 14px;background:rgba(255,255,255,0.04);border-radius:0 0 8px 8px;border-top:1px solid rgba(255,255,255,0.06);">
            <p style="margin:0 0 2px;font-size:9px;font-weight:700;text-transform:uppercase;letter-spacing:0.1em;color:rgba(255,255,255,0.3);">${credLabel}</p>
            <p style="margin:0;font-size:14px;font-weight:700;color:#a78bfa;font-family:monospace;">${opts.masterPassword}</p>
          </td></tr>
        </table>
      </td></tr>
    </table>
    ...
    <table cellpadding="0" cellspacing="0" style="width:100%;background:#fafafe;border:1.5px solid #f0eef9;border-radius:12px;margin-bottom:20px;"><tbody>
      ...
      ${infoRow("Fulfillment", opts.fulfillmentType)}
...
export function walletFundedEmail(opts: {
  ...
  return layout(`
    ...
    <p style="margin:0 0 20px;font-size:14px;color:#9b8fc2;line-height:1.7;">
      Hey <strong style="color:#1a1230;">@${opts.username}</strong> — your wallet has been funded.
    </p>
...
export function accountBannedEmail(username: string) {
  return layout(
    ...
    <p style="margin:0 0 20px;font-size:14px;color:#9b8fc2;line-height:1.7;">
      Hi <strong style="color:#1a1230;">@${username}</strong> — your account has been restricted due to a terms of service violation.
    </p>
...
export function accountRestoredEmail(username: string) {
  return layout(`
    ...
    <p style="margin:0 0 20px;font-size:14px;color:#9b8fc2;line-height:1.7;">
      Hi <strong style="color:#1a1230;">@${username}</strong> — your account has been fully restored. Welcome back!
    </p>
...
export function adminMessageEmail(opts: {
  ...
  return layout(
    `
    <h1 style="margin:0 0 8px;font-size:26px;font-weight:800;color:#1a1230;">${opts.subject}</h1>
    <p style="margin:0 0 20px;font-size:14px;color:#9b8fc2;">Hi <strong style="color:#1a1230;">@${opts.username}</strong>,</p>
    <div style="padding:20px 24px;background:#fafafe;border:1.5px solid #f0eef9;border-radius:14px;margin-bottom:24px;">
      <p style="margin:0;font-size:14px;color:#475569;line-height:1.8;white-space:pre-wrap;">${opts.message}</p>
    </div>
    ${divider()}
