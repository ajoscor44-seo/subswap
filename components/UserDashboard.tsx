...
const CredentialCard: React.FC<{ sub: UserSubscription }> = ({ sub }) => {
  ...
  return (
    <div className="bg-white border border-slate-200 rounded-2xl overflow-hidden shadow-sm p-6 flex flex-col sm:flex-row items-start sm:items-center space-y-4 sm:space-y-0 sm:space-x-6">
      <div className="h-16 w-16 bg-slate-100 rounded-xl flex-shrink-0 flex items-center justify-center overflow-hidden">
        <img src={sub.master_accounts?.icon_url} alt="" className="w-full h-full object-cover" />
      </div>
      
      <div className="flex-grow min-w-0">
        <h3 className="text-lg font-bold text-slate-900 truncate">{sub.master_accounts?.service_name}</h3>
        <p className="text-sm text-slate-500">Profile Name: <span className="font-semibold text-slate-700">{sub.assigned_profile_name}</span></p>
      </div>
        ...
        {!revealed ? (
          ...
          <div className="space-y-2 w-full max-w-xs">
            <div className="flex items-center space-x-2 bg-slate-50 border border-slate-200 rounded-lg px-3 py-2">
              ...
              <span className="text-sm font-mono text-slate-800 flex-grow truncate">{sub.master_accounts?.master_email}</span>
              <button 
                onClick={() => navigator.clipboard.writeText(sub.master_accounts?.master_email || '')}
            ...
            <div className="flex items-center space-x-2 bg-slate-50 border border-slate-200 rounded-lg px-3 py-2">
              <span className="text-xs font-bold text-slate-400 uppercase w-12">Pass</span>
              <span className="text-sm font-mono text-slate-800 flex-grow truncate">{sub.master_accounts?.master_password}</span>
              <button 
                onClick={() => navigator.clipboard.writeText(sub.master_accounts?.master_password || '')}
                className="text-indigo-600 hover:text-indigo-800 text-xs font-bold"
              >
                Copy
              </button>
...
export const UserDashboard: React.FC<{ user: User | null }> = ({ user }) => {
  return (
    <div>
      <div className="mb-8">
        <h1 className="text-3xl font-extrabold text-slate-900 tracking-tight">My Access</h1>
        <p className="text-slate-500 mt-1">Your purchased premium subscription slots and credentials.</p>
      </div>
