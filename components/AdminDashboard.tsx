...
import { MasterAccount, User, Transaction, ProductCategory, FulfillmentType } from '../types';
...
const PRESET_ICONS = [
  { name: 'Netflix', url: 'https://images.unsplash.com/photo-1522869635100-9f4c5e86aa37?auto=format&fit=crop&q=80&w=400' },
  { name: 'Spotify', url: 'https://images.unsplash.com/photo-1614680376593-902f74cf0d41?auto=format&fit=crop&q=80&w=400' },
  { name: 'Canva', url: 'https://images.unsplash.com/photo-1611162617474-5b21e879e113?auto=format&fit=crop&q=80&w=400' },
  { name: 'ChatGPT', url: 'https://images.unsplash.com/photo-1677442136019-21780ecad995?auto=format&fit=crop&q=80&w=400' },
  { name: 'YouTube', url: 'https://images.unsplash.com/photo-1611162616305-c69b3fa7fbe0?auto=format&fit=crop&q=80&w=400' },
];
...
const AdminDashboard: React.FC<AdminDashboardProps> = ({ user }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const payload = {
        ...formData,
        available_slots: editingId ? formData.available_slots : formData.total_slots
      };

      if (editingId) {
        const { error } = await supabase.from('master_accounts').update(payload).eq('id', editingId);
        if (error) throw error;
      } else {
  ...
  const handleDelete = async (id: string) => {
    if (!window.confirm("Are you sure you want to delete this log? It will disappear from the marketplace.")) return;
    setIsLoading(true);
    try {
  ...
  return (
    <div className="container mx-auto px-4 py-12 space-y-10">
      {feedback && (
        <div className={`fixed top-24 right-6 z-[300] px-6 py-4 rounded-2xl shadow-2xl animate-in slide-in-from-right-4 border flex items-center gap-3 ${
          feedback.type === 'success' ? 'bg-slate-900 border-slate-700 text-white' : 'bg-red-500 border-red-400 text-white'
        }`}>
          <i className={`fa-solid ${feedback.type === 'success' ? 'fa-circle-check text-emerald-400' : 'fa-circle-exclamation'}`}></i>
      ...
      <header className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
        <div>
          ...
          <p className="text-slate-500 font-medium">Master control for SubSwap inventory and members.</p>
        </div>
              ...
              <form onSubmit={handleSubmit} className="space-y-8">
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                  <div className="space-y-1">
                    ...
                    <input type="text" required className="w-full bg-slate-50 border border-slate-100 p-4 rounded-xl font-bold" value={formData.service_name} onChange={e => setFormData({...formData, service_name: e.target.value})} placeholder="e.g. Netflix Premium" />
                  </div>
                  
                  <div className="space-y-1">
                    <label className="text-[10px] font-black uppercase tracking-widest text-slate-400 ml-1">Category</label>
                    <select className="w-full bg-slate-50 border border-slate-100 p-4 rounded-xl font-bold" value={formData.category} onChange={e => setFormData({...formData, category: e.target.value as ProductCategory})}>
                  ...
                  <div className="space-y-1">
                    ...
                    <select className="w-full bg-slate-50 border-2 border-indigo-100 p-4 rounded-xl font-black text-indigo-600" value={formData.fulfillment_type} onChange={e => setFormData({...formData, fulfillment_type: e.target.value as FulfillmentType})}>
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                  ...
                  <div className="space-y-1">
                    ...
                    <input type="number" className="w-full bg-slate-50 border border-slate-100 p-4 rounded-xl font-bold" value={formData.original_price} onChange={e => setFormData({...formData, original_price: parseFloat(e.target.value)})} />
                  </div>
                  <div className="space-y-1">
                    <label className="text-[10px] font-black uppercase tracking-widest text-slate-400 ml-1">Total Slots</label>
                    <input type="number" required className="w-full bg-slate-50 border border-slate-100 p-4 rounded-xl font-bold" value={formData.total_slots} onChange={e => setFormData({...formData, total_slots: parseInt(e.target.value), available_slots: parseInt(e.target.value)})} />
                  </div>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6 p-6 bg-slate-50 rounded-3xl border border-dashed border-slate-200">
                  <div className="space-y-1">
                    <label className="text-[10px] font-black uppercase tracking-widest text-indigo-600 ml-1">
                      {formData.fulfillment_type === 'Invite Link' ? 'Activation Link' : 'Log Email / ID'}
                    </label>
                    <input 
                      type="text" 
                      required 
                      ...
                      placeholder={formData.fulfillment_type === 'Invite Link' ? "https://..." : "account@email.com"}
                    />
                  </div>
                  <div className="space-y-1">
                    <label className="text-[10px] font-black uppercase tracking-widest text-indigo-600 ml-1">
                      {formData.fulfillment_type === 'OTP / Instruction' ? 'Setup Note' : 'Log Password'}
                    </label>
                    <input 
                      type="text" 
                      required 
                      ...
                      placeholder={formData.fulfillment_type === 'OTP / Instruction' ? "Instructions for customer..." : "••••••••"}
                    />
                  </div>
                </div>

                <div className="space-y-3">
                   <label className="text-[10px] font-black uppercase tracking-widest text-slate-400 ml-1 block">Quick Icon Select</label>
                   <div className="flex flex-wrap gap-4">
                      {PRESET_ICONS.map(icon => (
                        ...
                        >
                          <img src={icon.url} className="h-12 w-12 rounded-xl object-cover" alt={icon.name} />
                        </button>
                      ))}
                      <input 
                        type="text" 
                        placeholder="Custom URL..." 
                        className="flex-grow bg-slate-50 border border-slate-100 p-4 rounded-xl font-bold text-xs"
                ...
                <div className="space-y-1">
                  <label className="text-[10px] font-black uppercase tracking-widest text-slate-400 ml-1">Description / Internal Notes</label>
                  <textarea className="w-full bg-slate-50 border border-slate-100 p-4 rounded-xl font-bold h-24" value={formData.description} onChange={e => setFormData({...formData, description: e.target.value})} placeholder="Tell users about this log..."></textarea>
                </div>

                <div className="flex gap-4">
                  <button type="submit" disabled={isLoading} className="flex-grow bg-slate-900 text-white py-5 rounded-2xl font-black uppercase tracking-widest shadow-xl hover:bg-indigo-600 transition-all disabled:opacity-50">
                    {isLoading ? <i className="fa-solid fa-spinner fa-spin mr-2"></i> : null}
                    {editingId ? 'Update Log' : 'Launch New Log'}
                  </button>
                  <button type="button" onClick={() => {setShowForm(false); setEditingId(null);}} className="px-8 bg-slate-100 text-slate-400 font-black uppercase tracking-widest rounded-2xl hover:bg-red-50 hover:text-red-500 transition-all">Cancel</button>
                </div>
              </form>
            </div>
          )}
              ...
              <table className="w-full text-left">
                ...
                <tbody className="divide-y divide-slate-100">
                    ...
                    <tr key={acc.id} className="hover:bg-slate-50 transition-colors group">
                      <td className="px-8 py-6">
                        <div className="flex items-center gap-4">
                           <img src={acc.icon_url} className="h-10 w-10 rounded-xl object-cover shadow-sm" alt="" />
                           <div className="flex flex-col">
                             <span className="font-black text-slate-900 text-sm">{acc.service_name}</span>
                             <span className="text-[9px] text-slate-400 uppercase tracking-widest font-black">{acc.category}</span>
                           </div>
                        </div>
                      </td>
                      <td className="px-8 py-6">
                      ...
                      <td className="px-8 py-6 max-w-[200px]">
                        <div className="flex flex-col">
                           <span className="text-xs font-bold text-slate-700 truncate">{acc.master_email}</span>
                           <span className="text-[9px] text-slate-300 font-mono tracking-widest mt-0.5">••••••••</span>
                        </div>
                      </td>
                      ...
                      <td className="px-8 py-6 text-right">
                        <div className="flex justify-end gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                          <button onClick={() => handleEdit(acc)} className="h-9 w-9 bg-slate-100 rounded-lg flex items-center justify-center text-slate-600 hover:bg-indigo-600 hover:text-white transition-all"><i className="fa-solid fa-pen-to-square"></i></button>
                          <button onClick={() => handleDelete(acc.id)} className="h-9 w-9 bg-slate-100 rounded-lg flex items-center justify-center text-slate-600 hover:bg-red-500 hover:text-white transition-all"><i className="fa-solid fa-trash"></i></button>
                        </div>
                      </td>
                    </tr>
                  ))}
                  {filteredData.length === 0 && (
                    <tr>
                      <td colSpan={6} className="px-8 py-20 text-center text-slate-400 font-black uppercase tracking-widest text-xs">No logs found matching your criteria.</td>
                    </tr>
                  )}
                </tbody>
              </table>
                      ...
                      <tr key={u.id} className={`hover:bg-slate-50 transition-colors ${u.is_banned ? 'opacity-50' : ''}`}>
                        <td className="px-8 py-6">
                          <div className="flex items-center gap-3">
                             <img src={u.avatar} className="h-9 w-9 rounded-full border border-slate-100" alt="" />
                             <div className="flex flex-col">
                               <span className="font-black text-slate-900 text-sm">@{u.username}</span>
                               <span className="text-[10px] text-slate-400 truncate max-w-[150px]">{u.email}</span>
                             </div>
                          </div>
                        </td>
        ...
        <div className="space-y-8 animate-in fade-in duration-300">
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            <div className="bg-white p-10 rounded-[3rem] border border-slate-200 shadow-sm relative overflow-hidden group">
              ...
              <p className="text-[10px] font-black uppercase tracking-[0.3em] text-slate-400 mb-2 relative">Platform Escrow</p>
              <h3 className="text-5xl font-black text-slate-900 relative tracking-tighter">₦{allUsers.reduce((a, b) => a + (Number(b.balance) || 0), 0).toLocaleString()}</h3>
            </div>
            <div className="bg-white p-10 rounded-[3rem] border border-slate-200 shadow-sm group relative">
              ...
              <p className="text-[10px] font-black uppercase tracking-[0.3em] text-slate-400 mb-2 relative">Registered Savers</p>
              <h3 className="text-5xl font-black text-slate-900 relative tracking-tighter">{allUsers.length}</h3>
            </div>
            <div className="bg-indigo-600 p-10 rounded-[3rem] text-white shadow-2xl shadow-indigo-200 relative overflow-hidden">
              ...
              <p className="text-[10px] font-black uppercase tracking-[0.3em] text-indigo-300 mb-2 relative">Active Logs</p>
              <h3 className="text-5xl font-black relative tracking-tighter">{accounts.length}</h3>
            </div>
          </div>
          
          <div className="bg-slate-900 rounded-[3rem] p-10 text-center">
             <h4 className="text-xl font-black text-white mb-2">Automated Inventory System</h4>
                ...
                <tbody className="divide-y divide-slate-100">
                  {filteredData.map((tx: any) => (
                    <tr key={tx.id} className="hover:bg-slate-50 transition-colors">
                      ...
                      <td className={`px-8 py-6 text-right font-black text-sm ${tx.amount < 0 ? 'text-red-500' : 'text-emerald-500'}`}>
                        {tx.amount > 0 ? '+' : ''}₦{tx.amount.toLocaleString()}
                      </td>
                      <td className="px-8 py-6 text-xs text-slate-500 font-medium">
                        <div className="flex flex-col">
                           <span className="text-slate-900 font-bold">{tx.description}</span>
                           <span className="text-[9px] text-slate-400">UID: ...{tx.user_id.slice(-8)}</span>
                        </div>
                      </td>
                  ...
                  {filteredData.length === 0 && (
                    <tr>
                      <td colSpan={4} className="px-8 py-20 text-center text-slate-400 font-black uppercase tracking-widest text-xs">No transaction records found.</td>
                    </tr>
                  )}
                </tbody>
             </table>
