...
const TransactionHistory: React.FC<TransactionHistoryProps> = ({ user, isDashboardView = false }) => {
      ...
      <div className={isDashboardView ? "" : "container mx-auto px-4"}>
        ...
        {isDashboardView && (
           <div className="p-8 border-b border-slate-100">
              <h3 className="text-2xl font-black text-slate-900 tracking-tight">Financial History</h3>
              <p className="text-slate-500 text-sm font-medium">Complete record of your wallet movements.</p>
           </div>
        )}
                  ...
                  <div className="flex justify-between items-center pt-1">
                    ...
                    {user.isAdmin && (
                      <span className="font-mono text-[8px] text-slate-400">
                        ID: {tx.user_id === user.id ? 'Self' : `...${tx.user_id.slice(-6)}`}
                      </span>
                    )}
                  </div>
                </div>
            ...
            <table className="w-full text-left">
              <thead>
                <tr className="bg-slate-50 border-b border-slate-100">
                  {user.isAdmin && <th className="px-8 py-6 text-[10px] font-black uppercase tracking-widest text-slate-400">User ID</th>}
              ...
              <tbody className="divide-y divide-slate-100">
                {isLoading ? (
                  ...
                  <tr>
                    <td colSpan={user.isAdmin ? 5 : 4} className="text-center py-20 text-slate-400 font-bold uppercase tracking-widest text-[10px]">
