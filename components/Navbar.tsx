...
const Navbar: React.FC<NavbarProps> = ({ user, currentView, onNavigate, onLogin, onLogout }) => {
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex justify-between h-20">
          <div className="flex items-center gap-6 lg:gap-10">
            ...
            >
              <div className="h-10 w-10 bg-indigo-600 rounded-xl flex items-center justify-center text-white font-black text-xl transition-transform group-hover:scale-110">S</div>
              <span className="text-xl font-black text-slate-900 tracking-tighter hidden sm:inline">SUBSWAP</span>
            </div>

            <div className="hidden lg:flex items-center gap-1">
              <button 
                onClick={() => onNavigate('home')}
                className={`px-4 py-2 rounded-lg text-sm font-bold transition-all ${currentView === 'home' ? 'bg-indigo-50 text-indigo-600' : 'text-slate-500 hover:text-slate-900 hover:bg-slate-50'}`}
              >
                Marketplace
              </button>
              <button 
            ...
            {user ? (
              <div className="flex items-center gap-2 md:gap-4">
                ...
                <div className="relative">
                  ...
                  >
                    <img src={user.avatar} className="h-8 w-8 rounded-full border border-white shadow-sm" alt="Avatar" />
                    <div className="text-left hidden sm:block">
                      <p className="text-xs font-black text-slate-900 leading-none">₦{user.balance.toLocaleString()}</p>
                      <p className="text-[10px] font-bold text-slate-400 uppercase tracking-tighter mt-0.5">Balance</p>
                    </div>
                  ...
                  {isDropdownOpen && (
                    <div className="absolute right-0 mt-3 w-56 bg-white rounded-2xl shadow-2xl border border-slate-200 py-2 animate-in fade-in slide-in-from-top-2">
                      ...
                      <button onClick={() => { onNavigate('dashboard'); setIsDropdownOpen(false); }} className="w-full text-left px-4 py-2.5 text-sm font-bold text-slate-700 hover:bg-slate-50 transition-colors">
                        <i className="fa-solid fa-wallet mr-2"></i> Wallet & Fund
                      </button>
                      <button onClick={() => { onNavigate('transactions'); setIsDropdownOpen(false); }} className="w-full text-left px-4 py-2.5 text-sm font-bold text-slate-700 hover:bg-slate-50 transition-colors">
                        <i className="fa-solid fa-receipt mr-2"></i> History
                      </button>
                      <button onClick={() => { onNavigate('dashboard'); setIsDropdownOpen(false); }} className="w-full text-left px-4 py-2.5 text-sm font-bold text-slate-700 hover:bg-slate-50 transition-colors">
                        <i className="fa-solid fa-layer-group mr-2"></i> My Stacks
                      </button>
                      {user.isAdmin && (
                        <button onClick={() => { onNavigate('admin'); setIsDropdownOpen(false); }} className="w-full text-left px-4 py-2.5 text-sm font-black text-indigo-600 hover:bg-indigo-50 transition-colors">
                          <i className="fa-solid fa-gauge-high mr-2"></i> Admin Dashboard
                        </button>
                      )}
                      <button onClick={onLogout} className="w-full text-left px-4 py-2.5 text-sm font-bold text-red-500 hover:bg-red-50 transition-colors">
