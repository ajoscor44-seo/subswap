...
export const JoinConfirmation: React.FC<JoinConfirmationProps> = ({ subscription, onClose }) => {
  ...
  useEffect(() => {
    const getProrated = async () => {
      ...
      if (groups && groups.length > 0) {
        ...
        const fullPrice = subscription.monthly_price / subscription.max_seats;
        setProratedPrice({
        ...
        <div className="text-center mb-8">
          <div className="h-20 w-20 rounded-3xl bg-slate-50 flex items-center justify-center border border-slate-100 mx-auto mb-4 overflow-hidden">
            {subscription.logo_url ? (
              <img src={subscription.logo_url} alt={subscription.name} className="h-14 w-14 object-contain" />
            ) : (
              <i className="fa-solid fa-layer-group text-slate-300 text-3xl" />
            )}
          </div>
          <h2 className="text-2xl font-black text-slate-900 font-display">{subscription.name}</h2>
          <p className="text-slate-500 text-sm mt-1">Confirm your seat in this shared plan</p>
        </div>

        <div className="space-y-4 mb-8">
          <div className="flex justify-between items-center p-4 bg-slate-50 rounded-2xl border border-slate-100">
            <span className="text-xs font-bold text-slate-500 uppercase tracking-widest">Seat Price</span>
            <span className="text-sm font-black text-slate-900 font-display">₦{proratedData.fullRenewalPrice.toLocaleString()} / mo</span>
          </div>
          ...
          <div className="flex justify-between items-center p-4 bg-indigo-50 rounded-2xl border border-indigo-100">
            <div>
              ...
              <span className="text-[10px] text-indigo-400 font-medium">Prorated for {proratedData.remainingDays} days</span>
            </div>
            <span className="text-xl font-black text-indigo-700 font-display">₦{proratedData.proratedPrice.toLocaleString()}</span>
          </div>

          <div className="p-4 rounded-2xl border border-slate-100 text-center">
            <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-1">Next Billing</p>
            <p className="text-xs font-black text-slate-700 font-display">
              ₦{proratedData.fullRenewalPrice.toLocaleString()} on {new Date(new Date().getTime() + proratedData.remainingDays * 24 * 60 * 60 * 1000).toLocaleDateString()}
            </p>
          </div>
        </div>
