...
export const Layout: React.FC<LayoutProps> = ({ children, user, activeTab, onTabChange }) => {
  return (
    <div className="min-h-screen bg-slate-50 flex flex-col">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between h-16">
            <div className="flex items-center space-x-8">
              <div className="flex-shrink-0 flex items-center cursor-pointer" onClick={() => onTabChange('marketplace')}>
                <span className="text-2xl font-black text-indigo-600 tracking-tighter">SUBSWAP</span>
              </div>
              ...
              {user ? (
                <div className="flex items-center space-x-4">
                  <div className="text-right hidden sm:block">
                    ...
                    <p className="text-xs text-slate-500 uppercase tracking-widest">{user.isAdmin ? 'Admin' : 'User'}</p>
                  </div>
      ...
      <footer className="bg-white border-t border-slate-200 py-8">
        <div className="max-w-7xl mx-auto px-4 text-center text-slate-400 text-sm">
          &copy; {new Date().getFullYear()} SubSwap Subscription Marketplace.
        </div>
      </footer>
    </div>
  );
